...
TEST (network, checkFeedForwardOutputs_FourTotalOutput){
    ...
    */
    ...
    MatrixXd layerThreeOutputs(3,1);
    layerThreeOutputs << 0.15126992421132374, 0.98848562983469246, 0.00076072231080266943;
    MatrixXd layerFourOutputs(4,1);
    layerFourOutputs << -1.82341916853, 4.33278411846, -6.8421490684, 9.35151401834;
    ...
    //check the final outputs (rechecks)
    EXPECT_NEAR(net->network[net->network.size()-1].outputs.sum(),outputs[net->network.size()-1].sum(),error);
}


TEST (network, DISABLED_checkErrors){
    Network *net = TestInitialization::net;
    double error = 0.00000001;
...
TEST (network, checkErrors_FourOutputs){
    Network *net = TestInitialization::net;
    double error = 0.00000001;
    ...
    MatrixXd desiredOutput(4,1);
    desiredOutput << 1,0.5,-0.5,0;
    net->backpropogate(inputs, desiredOutput);
